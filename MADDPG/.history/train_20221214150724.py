...
learning_start_step = int(2e5)  # 开始训练的游戏步数
...
Num_hidden_1 = 128
Num_hidden_2 = 128
BUFFER_SIZE = 1e6  # 5000
max_episode = 50000   # 1000
...
def get_trainers(env, obs_shape_n, action_shape_n, num_hidden_1, num_hidden_2):
    ...
    for i in range(env.world.num_UAVs):
        ...
        optimizers_a[i] = optim.Adam(actors_cur[i].parameters(), lr=learning_rate_a)
        optimizers_c[i] = optim.Adam(critics_cur[i].parameters(), lr=learning_rate_c)
...
def train(var):
    ...
    for episode_gone in range(1,max_episode+1):
        duration = 0
        while True:
            # get action
            action_n = None
            # l = open('game step log.txt', 'a+')
            # print('game_step:{}'.format(game_step),file=l)
            if game_step < learning_start_step:
                ...
                # else:
                action_n = [
                        agent(torch.from_numpy(obs).to(device, torch.float)).detach().cpu().numpy() + np.random.randn(3)*var for agent, obs in zip(actors_cur, obs_n)]
            # 和环境交互
        ...
        print('episode_rewards[-1]:{}'.format(episode_rewards[-1]))
        mean_ep_r = round(np.mean(episode_rewards[:-1]), 3)
        mean_agents_r = [round(np.mean(agent_rewards[idx][:-1]), 2) for idx in range(env.world.num_UAVs)]
        print(" " * 43 + 'episode reward:{}    agent_rewards:{}'.format(mean_ep_r, mean_agents_r))
        print(var)

        if episode_gone % 50000 == 0:
