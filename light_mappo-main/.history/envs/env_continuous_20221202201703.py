...
from envs.multiUAV_sceneario import Scenario
Range = 600

class ContinuousActionEnv(object):
    """对于动作环境的封装"""
    def __init__(self, sceneario):
        # self.env = EnvCore()
        self.env = sceneario
    ...
    def step(self, actions):
        ...
        # infos是计算出来的jain fairness index
    ...
    def get_Jain_Index(self):
        ans = self.env.get_Jain_index(self.world)
        return ans
...
if __name__ == "__main__":
    sc = Scenario()
    env = ContinuousActionEnv(sc)
    env.reset()
    env.render()
    while True:
        obs_n, reward, done, infos = env.step()
        if env.world.t == 200:
            break;
