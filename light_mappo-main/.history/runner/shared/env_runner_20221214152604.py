    ...
    def run(self):
        ...
        for episode in range(episodes):
            ...
            if episode % self.log_interval == 0:
                ...
                print('self.buffer.rewards.length:{}'.format(len(self.buffer.rewards)))
                train_infos["average_episode_rewards"] = np.mean(self.buffer.rewards) * self.episode_length
                # l=open('reward日志{0}.txt'.format(1),'w')
                print('self.buffer.rewards:{}'.format(self.buffer.rewards), file=l)
                print("average episode rewards is {}".format(train_infos["average_episode_rewards"]))
                epochs.append(episode)
                plot_res.append(train_infos["average_episode_rewards"])
                self.log_train(train_infos, total_num_steps)
                # plt.plot(episode, train_infos["average_episode_rewards"])
                # plt.xlabel('epoch')
